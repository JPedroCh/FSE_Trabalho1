    ...
    while True:
        ...
        for sala in salas_info:
          ...
          # sala selecionada
            ...
            message = build_command_message(decoded_message[0], decoded_message[1])
            break
        command = -1
...
def activate_alarm_system(conn):
...
def deactivate_alarm_system(conn):
    global alarm_system
    alarm_system = not alarm_system
    if(alarm_system):
...
def init_socket(host: str, port: int):
  ...
  while True:
      ...
      send_command_thread = Thread(target=send_command, args=(conn,))
...
def render_interface(stdscr):
    ...
    while(command != curses.ascii.ESC):
        ...
        if(len(salas_info) >= 2):
            ...
            stdscr.addstr(5, mid_x+margin_x, f"Quantidade de pessoas na sala: {qtd_pessoas_sala1 + qtd_pessoas_sala0} ", curses.color_pair(2))
            stdscr.addstr(5, margin_x, f"Quantidade de pessoas na sala: {qtd_pessoas_sala1 + qtd_pessoas_sala0} ", curses.color_pair(2))
